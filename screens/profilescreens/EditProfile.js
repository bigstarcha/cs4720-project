...
const EditProfile = ({route, navigation}) => {
  const {epronoun, eposition, efirstPromptQuestion, esecondPromptQuestion} =
    route.params;
  console.log(epronoun, eposition, efirstPromptQuestion, esecondPromptQuestion);
  useEffect(() => {
    ...
    db.get().then(doc => {
      if (doc.exists) {
        ...
        setFirstPromptAnswer(doc.data().firstPromptA);
        setSecondPromptAnswer(doc.data().secondPromptA);
        setName(doc.data().name);
        setCompany(doc.data().company);
        setImage(doc.data().imageUri);
      } else {
  ...
  const [position, setPosition] = useState(eposition); // Position
  const [isDatePickerVisible, setDatePickerVisibility] = useState(false); // Date picker visibility
  const [firstPromptQuestion, setFirstPromptQuestion] =
    useState(efirstPromptQuestion); // First prompt question
  const [secondPromptQuestion, setSecondPromptQuestion] = useState(
    esecondPromptQuestion,
  ...
  const editProfile = () => {
    ...
    if (
      ...
      } else if (secondPromptQuestion == '') {
        alert('Please enter your second prompt question');
      } else if (secondPromptAnswer == '') {
        alert('Please enter your second prompt answer');
      }
    ...
    //if image starts with https://, then skip uploading
    if (!image.startsWith('https://')) {
      ...
      task.then(() => {
        ...
        const newImageUri =
          'https://firebasestorage.googleapis.com/v0/b/project-thc8pku.appspot.com/o/' +
  ...
  function getKeyByValue(keyValuePairs, value) {
    for (let i = 0; i < keyValuePairs.length; i++) {
      if (keyValuePairs[i].value === value) {
        return keyValuePairs[i].key;
      }
    }
    return null;
  }
    ...
    <View style={styles.mainView}>
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.profileDOB}>
          <View>
            <Text style={{fontSize: 20, marginBottom: 2}}>Date of Birth</Text>
        ...
        <View style={styles.profilePronouns}>
          <Text style={{fontSize: 20, marginBottom: 5}}>Pronouns</Text>
          <SelectList
            setSelected={val => setPronoun(val)}
            data={pronouns}
            search={false}
            save="value"
            defaultOption={pronouns[getKeyByValue(pronouns, epronoun)]} // Figured out prepopulating the dropdown menu
          />
        </View>

        {/** Profile Position */}
        <View style={styles.profilePosition}>
          <Text style={{fontSize: 20, marginBottom: 5}}>Position</Text>
          <SelectList
            setSelected={val => setPosition(val)}
            data={positions}
            search={false}
            save="value"
            defaultOption={positions[getKeyByValue(positions, eposition)]}
          />
        </View>

        {/** Profile Company */}
        <View style={styles.profileCompany}>
          <Text style={{fontSize: 20, marginBottom: 5}}>Company</Text>
          <TextInput
            style={{
              height: 40,
              fontSize: 15,
        ...
        <View style={styles.profileLocation}>
          <Text style={{fontSize: 20, marginBottom: 5}}>Location</Text>
          <Text style={{fontSize: 15}}>{location}</Text>
        </View>

        {/** Profile First prompt and answer */}
        <View>
          <Text style={{fontSize: 20, marginBottom: 5}}>Prompts</Text>
          ...
          <SelectList
            ...
            save="value"
            defaultOption={
              firstPromptQuestions[
                getKeyByValue(firstPromptQuestions, efirstPromptQuestion)
              ]
            }
          />
          ...
          <SelectList
            ...
            save="value"
            defaultOption={
              secondPromptQuestions[
                getKeyByValue(secondPromptQuestions, esecondPromptQuestion)
              ]
            }
          />
      ...
      <TouchableOpacity
        ...
        onPress={editProfile}>
        <Text style={{fontSize: 25}}>Update Profile</Text>
      </TouchableOpacity>
    </View>
  );
};
